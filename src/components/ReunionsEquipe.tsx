...
const ReunionsEquipe: React.FC = () => {
  ...
  const filteredSujets = sujets.filter(sujet => {
    const matchesSearch = sujet.titre.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         sujet.responsable.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      <ConfigurationSection title="Réunions Équipe" />
      <div className="flex items-center justify-between">
        <h2 className="text-3xl font-bold text-gray-900">Stand-up Équipe</h2>
        <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{sujets.length}</p>
            </div>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-600">
                        <strong>Responsable:</strong> {sujet.responsable}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Calendar size={16} className="text-gray-400" />
                      <span className="text-sm text-gray-600">
                        <strong>Stand-up:</strong> {formatDate(sujet.dateReunion)}
                      </span>
                    </div>
                  </div>
      ...
      {filteredSujets.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Commencez par créer un nouveau sujet pour votre stand-up équipe.
          </p>
        </div>
      )}
